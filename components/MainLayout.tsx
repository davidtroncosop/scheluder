    ...
    return (
        <div className="flex h-screen w-full overflow-hidden bg-background-light dark:bg-background-dark font-display transition-colors duration-500">
            ...
            <nav className={`
                ...
                bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-r border-slate-200/60 dark:border-white/5 
                flex flex-col shrink-0 py-6 transition-all duration-300 shadow-2xl md:shadow-none
            `}>
                {/* Desktop Toggle Button */}
                <button
                    onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
                    className="hidden md:flex absolute -right-3 top-12 size-7 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 rounded-full items-center justify-center shadow-md z-50 hover:bg-slate-50 hover:scale-105 active:scale-95 dark:hover:bg-slate-700 transition-all text-slate-500 dark:text-slate-400 group"
                ...
                <button
                    onClick={() => setMobileMenuOpen(false)}
                    className="md:hidden absolute right-4 top-5 size-8 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white flex items-center justify-center"
                >
                    <span className="material-symbols-outlined text-xl">close</span>
                </button>

                <div className={`${sidebarCollapsed ? 'md:px-4' : 'px-8'} px-6 mb-6`}>
                    <Link to="/scheduler" className={`flex items-center ${sidebarCollapsed ? 'md:justify-center' : 'gap-3'} text-primary mb-5 overflow-hidden group`}>
                        ...
                        {(!sidebarCollapsed || mobileMenuOpen) && (
                            <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white group-hover:text-primary-dark transition-colors whitespace-nowrap">
                                Scheduler Pro
                            </h1>
                        )}
                    </Link>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    <div className={`${sidebarCollapsed ? 'md:px-4 md:text-center' : 'px-8'} px-6 mb-2`}>
                        {(!sidebarCollapsed || mobileMenuOpen) && <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1">Principal</p>}
                        {sidebarCollapsed && !mobileMenuOpen && <div className="h-px w-6 mx-auto bg-slate-200 dark:bg-slate-700"></div>}
                    </div>

                    <ul className={`space-y-1.5 ${sidebarCollapsed ? 'md:px-3' : 'px-4'} px-4`}>
                        {mainNavItems.map(item => {
                            ...
                            return (
                                <li key={item.name}>
                                    <Link
                                        to={item.path}
                                        className={`w-full flex items-center ${sidebarCollapsed ? 'md:justify-center' : ''} gap-3 ${sidebarCollapsed ? 'md:p-3' : 'px-4 py-2.5'} px-4 py-2.5 text-[15px] font-medium rounded-xl transition-all group relative overflow-hidden ${isActive
                                            ? 'text-primary dark:text-primary-light bg-primary/10 dark:bg-primary/20 shadow-xs font-bold'
                                    ...
                                    >
                                        ...
                                        {(!sidebarCollapsed || mobileMenuOpen) && <span className="whitespace-nowrap">{item.name}</span>}
                    ...
                    <div className={`${sidebarCollapsed ? 'md:px-4 md:text-center' : 'px-8'} px-6 mt-8 mb-2`}>
                        {(!sidebarCollapsed || mobileMenuOpen) && <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1">Maestros</p>}
                        {sidebarCollapsed && !mobileMenuOpen && <div className="h-px w-6 mx-auto bg-slate-200 dark:bg-slate-700"></div>}
                    </div>
                    <ul className={`space-y-1.5 ${sidebarCollapsed ? 'md:px-3' : 'px-4'} px-4`}>
                    ...
                    <div className={`${sidebarCollapsed ? 'md:px-4 md:text-center' : 'px-8'} px-6 mt-8 mb-2`}>
                        {(!sidebarCollapsed || mobileMenuOpen) && <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1">Sistema</p>}
                        {sidebarCollapsed && !mobileMenuOpen && <div className="h-px w-6 mx-auto bg-slate-200 dark:bg-slate-700"></div>}
                    </div>
                    <ul className={`space-y-1.5 ${sidebarCollapsed ? 'md:px-3' : 'px-4'} px-4 pb-4`}>
                        <li>
                            <Link
                                to="/upload"
                                className={`w-full flex items-center ${sidebarCollapsed ? 'md:justify-center' : ''} gap-3 ${sidebarCollapsed ? 'md:p-3' : 'px-4 py-2.5'} px-4 py-2.5 text-[15px] font-medium rounded-xl transition-all group ${location.pathname === '/upload'
                                    ? 'text-primary dark:text-primary-light bg-primary/10 dark:bg-primary/20 shadow-xs font-bold'
                                    : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5'
                                    }`}
                            ...
                            >
                                ...
                                {(!sidebarCollapsed || mobileMenuOpen) && <span className="whitespace-nowrap">Importar Datos</span>}
                            </Link>
                        </li>
                    </ul>
                </div>

                <div className={`mt-auto pt-4 border-t border-slate-100 dark:border-white/5 ${sidebarCollapsed ? 'md:px-3' : 'px-4'} px-4`}>
                    <Link
                        to="/configuracion"
                        className={`w-full flex items-center ${sidebarCollapsed ? 'md:justify-center' : ''} gap-3 ${sidebarCollapsed ? 'md:p-3' : 'px-4 py-2.5'} px-4 py-2.5 text-[15px] font-medium rounded-xl transition-all group ${location.pathname === '/configuracion' ? 'bg-primary/5 text-primary font-bold' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5'}`}
                        title={sidebarCollapsed ? 'Configuración' : undefined}
                    >
                        <span className="material-symbols-outlined text-[20px] group-hover:rotate-45 transition-transform">settings</span>
                        {(!sidebarCollapsed || mobileMenuOpen) && <span className="whitespace-nowrap">Configuración</span>}
                    </Link>
                    <Link
                        to="/"
                        onClick={() => session.clear()}
                        className={`w-full flex items-center ${sidebarCollapsed ? 'md:justify-center' : ''} gap-3 ${sidebarCollapsed ? 'md:p-3' : 'px-4 py-2.5'} px-4 py-2.5 mt-1 text-[15px] font-medium rounded-xl transition-all group text-slate-500 dark:text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10`}
                        title={sidebarCollapsed ? 'Cerrar Sesión' : undefined}
                    >
                        <span className="material-symbols-outlined text-[20px] group-hover:-translate-x-1 transition-transform">logout</span>
                        {(!sidebarCollapsed || mobileMenuOpen) && <span className="whitespace-nowrap">Cerrar Sesión</span>}
                    </Link>
                </div>
            </nav>
